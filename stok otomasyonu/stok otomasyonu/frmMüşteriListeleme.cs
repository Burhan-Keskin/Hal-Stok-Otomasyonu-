...
namespace stok_otomasyonu
{
    public partial class frmMüşteriListeleme : Form
    {
        ...
        {
            baglantı.Open();
            SqlCommand komut = new SqlCommand("update müşteri sed adsoyad=@adsoyad,telefon,=@telefon,adres=@adres,email=@email where tc=@tc",baglantı);
        ...
        {
            baglantı.Open();
            SqlCommand komıut = new SqlCommand("delete from müşteri where tc='"+dataGridView1.CurrentRow.Cells["tc"].Value.ToString()+"'",baglantı);
